...
exports.protect = asyncErrorHandler(async (req, res, next) => {
    ...
    try {
        ...
        if (users.length === 0) {
            // Usuń nieprawidłową sesję
            req.session.destroy();
            const error = new CustomError('Użytkownik przypisany do tej sesji już nie istnieje.', 401);
            return next(error);
        }
...
exports.logout = asyncErrorHandler(async (req, res, next) => {
    req.session.destroy((err) => {
        ...
        res.clearCookie('connect.sid'); // Usuń cookie sesji (domyślna nazwa)
        res.status(200).json({
            status: 'success',
